...
public static void main(String[] args) {
	...
	File f= new File("/Users/shenlei/Temp/imagescale/1920x1080.jpg");
	System.out.println(f.length());
	//System.out.println(Math.sqrt((double)f.length()/(10*1024)));
	double scale = Math.sqrt(f.length()/(10*1024));
	
	System.out.println(scale);
	
	ImageUtils.scale("/Users/shenlei/Temp/imagescale/1920x1080.jpg", "/Users/shenlei/Temp/imagescale/1920x1080_small.jpg", scale, false);//测试OK
 ...
 * @param size 目标大小
 */
public final static void thumbnail(String srcImageFile, String outFile,
        long imageSize) {
	File srcFile= new File(srcImageFile);	
	double scale = Math.sqrt(srcFile.length()/imageSize);
	
	if(scale<1){
		scale = 1;
	}
	ImageUtils.scale(srcImageFile, outFile, scale, false);
}
 ...
 * 2018年1月8日
 */
public static void cutImage(File image, int x, int y, int width, int height)
		throws IOException {
	//避免截图的时候的长宽度大于图片的长宽产生异常
	try{
		...
		RenderedImage renderedImage = bufferedImage.getSubimage(x, y, width,height);
		ImageIO.write(renderedImage, "jpg", image);
	}catch(Exception e){
