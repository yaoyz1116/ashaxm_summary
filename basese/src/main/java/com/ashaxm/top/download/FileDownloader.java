...
public class FileDownloader {
	...
	public void execute(){
		...
		try {
			cm.open(url);
			List<ConnectionImpl> poll = ConnectionManagerImpl.getPoll();
			for(ConnectionImpl conn : poll) {
				System.out.println("启动线程-----"+conn.getStartPos());
				new DownloadThread(conn,url, conn.getStartPos(), conn.getStartPos()+conn.getContentLength()).start();
			}
		} catch (Exception e) {			
			e.printStackTrace();
		}finally{
